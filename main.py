...
from get_play_version import get_version
import mongo_op
...
def main():
    ...
    parser.add_argument(
        "-t", "--token", type=str, default="", help="Your firebase refreshToken."
    )
    parser.add_argument(
        "-f",
        "--force",
        action="store_true",
        help="Update databases without checking version.",
    )
    parser.add_argument(
        "-fk",
        "--force-kv",
        action="store_true",
        help="Put databases without checking version.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Enable asset decryption mode. Can be either (all | diff).",
    )
    parser.add_argument(
        "-o",
        "--overwrite",
        action="store_true",
        help="Overwrite cached token. If '--token' does not exist, this argument takes no effect.",
    )
    ...
    parser.add_argument(
        "--venus",
        action="store_true",
        help="Check and update(if needed) hoshimi-venus databases locally. Pushing to remote repository is not its duty.",
    )
    parser.add_argument(
        "--gp",
        action="store_true",
        help="Use python to get app version from Google Store.",
    )
    parser.add_argument(
        "--pushmongo", action="store_true", help="Push databases to hoshimi-db."
    )
    parser.add_argument(
        "--mongouri", type=str, default="", help="Mongodb connection string."
    )
    ...
    if args.gp:
        version = get_version()
        Path("cache/appversion").write_text(version)
    ...
    with SolisClient() as client:
        ...
        if args.venus:
            ...
            if need_update:
                ...
                Path("cache/need_update.txt").unlink(missing_ok=True)
            # use this line to force update venus if needed
            # Path("cache/need_update.txt").write_text("1", encoding="ascii")
        if args.pushmongo and args.mongouri != "":
            asyncio.run(
                mongo_op.check_version_and_push_to_mongodb(args.mongouri, client)
            )
