...
class SolisClient(ClientBase):
    ...
    home_enter_res: apip.HomeEnterResponse = None
    pvp_top_res: apip.PvpTopResponse = None
    gvg_top_res: apip.GvgTopResponse = None
    league_top_res: apip.LeagueTopResponse = None
    marathon_top_res: apip.MarathonTopResponse = None
    _marathon_id: str = None
    ...
    def put_master(self, force: bool = False):
        kv_master_version = get_cache("kvMasterVersion")
        console.info(f"KV master version: '{kv_master_version}'.")
        if not force and kv_master_version == self.master_tag.version:
            console.info(
                "Kv master version is already up-to-date, put operation will not be performed."
            )
            return
        upload.main()
    ...
    def put_octo(self):
        if not get_cache("octoManifestRevision") < self.octo_server_revision:
            console.info(
                "OctoManifestRevision is already up-to-date, json string won't be put to KV."
            )
            return
    ...
    def run_login_scenarios(self, retries=0):
        ...
        if err == 0:
            ...
            penum.ErrorCode.ErrorCode_OutdatedMasterData,
    ...
    def post_token(self) -> int:
        ...
        request = json.dumps(
            {"grantType": "refresh_token", "refreshToken": get_cache("refreshToken")}
        )
    ...
    def first_system_check(self) -> int:
        ...
        response, _, err = self._call_rpc(
            stub.Check.with_call, apip.SystemCheckRequest(), metadata
        )
        return err

    def system_check(self) -> int:
        stub = apig.SystemStub(self._channel)
        request = apip.SystemCheckRequest(firebaseIDToken=get_cache("idToken"))
        response, _, err = self._call_rpc(stub.Check.with_call, request)
        return err

    def auth_login(self) -> int:
        stub = apig.AuthStub(self._channel)
        request = apip.AuthLoginRequest(firebaseIDToken=get_cache("idToken"))
    ...
    def home_login(self) -> int:
        ...
        response, _, err = self._call_rpc(stub.Login.with_call, request, metadata)
        return err
    ...
    def home_enter(self) -> int:
        ...
        metadata = self._get_metadata_pairs()
        metadata.append(("x-app-request-id", str(self._get_ticks())))
        response, _, err = self._call_rpc(stub.Enter.with_call, request, metadata)
        self.home_enter_res = response
        return err
    ...
    def league_top(self) -> int:
        stub = apig.LeagueStub(self._channel)
        request = empty.Empty()
        response, _, err = self._call_rpc(stub.Top.with_call, request)
        self.league_top_res = response
        return err

    def marathon_top(self) -> int:
        stub = apig.MarathonStub(self._channel)
        request = apip.MarathonTopRequest(marathonId=self._marathon_id)
        response, _, err = self._call_rpc(stub.Top.with_call, request)
        self.marathon_top_res = response
        return err

    def check_battle_data(self):
        if self.home_enter_res.HasField("pvpInfo"):
            now = int(time.time() * 1000)
            if (
                now < self.home_enter_res.pvpInfo.latestPvpSeason.endTime
    ...
    def update_battle_data(self) -> bool:
        need_update = False
        venus.prepare_battles()
        if self.pvp_top_res != None:
            need_update |= venus.update_battles(self.pvp_top_res, "pvp")
        if self.league_top_res != None:
            need_update |= venus.update_battles(self.league_top_res, "league")
        if self.gvg_top_res != None:
            need_update |= venus.update_battles(self.gvg_top_res, "gvg")
        if self.marathon_top_res != None:
            need_update |= venus.update_battles(self.marathon_top_res, "raid")
        return need_update

    def check_and_update_battle_data(self):
        self.check_battle_data()
        self.battle_updated = self.update_battle_data()
